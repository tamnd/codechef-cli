// Package cli builds the cc command tree on top of the codechef library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: codechef.DefaultConfig()}

	root := &cobra.Command{
		Use:   "cc",
		Short: "Browse CodeChef competitive programming problems",
		Long: `cc reads CodeChef through the public JSON APIs.
No API key required. Returns problems, contests, and search results
as table, JSON, JSONL, CSV, TSV, or URLs.

cc is an independent tool and is not affiliated with CodeChef or Directi.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = codechef.NewClient(a.cfg)
	return nil
}
