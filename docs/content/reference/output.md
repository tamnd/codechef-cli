...
describes what users get. Pick a format with `-o`, or let cc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cc <command> -o table   # aligned columns for reading
cc <command> -o jsonl   # one JSON object per line, for piping
cc <command> -o json    # a single JSON array
cc <command> -o csv     # spreadsheet friendly
cc <command> -o tsv     # tab-separated
cc <command> -o url     # just the URL column
cc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cc <command> --fields id,title,url
```
...
```bash
cc <command>            # a table, because this is a terminal
cc <command> | wc -l    # JSONL, because this is a pipe
```
